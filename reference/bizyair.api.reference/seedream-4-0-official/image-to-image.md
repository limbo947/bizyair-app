...
async function submitTask() {
  const url = 'https://api.bizyair.cn/x/v1/modelzoo/tasks/openapi/seedream-4-0-official/image-to-image';
  const payload = {
    "prompt": "钢铁侠小手办置于微型奇幻森林中，周围有发光蘑菇和微小精灵环绕，晨雾轻绕，奇幻风格，柔和光线，细节丰富，魔法光效，超高清渲染",
    "image_urls": [
...
| size | string | 否 | ⟨bz_enum_json⟩["1K","2K","4K"]⟨/bz_enum_json⟩<br/>指定生成图像的分辨率，并在prompt中用自然语言描述图片宽高比、图片形状或图片用途，最终由模型判断生成图片的大小。 |
...
```json
{
  ...
  "outputs": {
    "images": [
      "https://storage.bizyair.cn/outputs/iV9kL4Ur5rwhqZr5.jpeg"
    ]
  }
}
```
