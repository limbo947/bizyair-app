---
display_name: "Vidu Q3.Pro-图生视频-官方版"
category: "Image to Video"
manufacturer: "生数科技"
price: "启用推荐提示词时，额外收取一次320金币费用"
price_url: "https://bizyair.cn/modelzoo/vidu-q3-pro-official/image-to-video?tab=price"
benefit:
  rpd: 200
  rph: 60
  rpm: -1
description: |
  生数科技Q3.Pro官方版图像转视频模型，在精准识别图像内容的基础上，可生成更高质量的连贯动态画面，主体场景还原度高，画面稳定无畸变漂移，适配多风格创作需求，兼顾写实与二次元画风，适合高品质短视频、创意素材及各类视频制作需求。
tags: ["Vidu"]
---

# Vidu Q3.Pro-图生视频-官方版

> **图生视频** | 厂商: 生数科技 | 模型: `vidu-q3-pro-official` | 类型: `image-to-video`

生数科技Q3.Pro官方版图像转视频模型，在精准识别图像内容的基础上，可生成更高质量的连贯动态画面，主体场景还原度高，画面稳定无畸变漂移，适配多风格创作需求，兼顾写实与二次元画风，适合高品质短视频、创意素材及各类视频制作需求。

💰 **价格**: 启用推荐提示词时，额外收取一次320金币费用  [查看详情](https://bizyair.cn/modelzoo/vidu-q3-pro-official/image-to-video?tab=price)
...
async function submitTask() {
  const url = 'https://api.bizyair.cn/x/v1/modelzoo/tasks/openapi/vidu-q3-pro-official/image-to-video';
  const payload = {
    "image": [
      "https://bizyair-prod.oss-cn-shanghai.aliyuncs.com/inputs/20260513/u6kSnCG7jncLBqjFOHg5G12VraMN5vbg.jpg?uploads="
    ],
    "last_frame_image": [],
...
| 参数名 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| image | array | 是 | 支持格式：png、jpeg、jpg、webp<br/>单文件大小上限：50.0 MB（52428800 byte）<br/>输入首帧图片，作为视频生成的起始帧。宽高比须在1:4~4:1之间 |
| last_frame_image | array | 否 | 支持格式：png、jpeg、jpg、webp<br/>单文件大小上限：50.0 MB（52428800 byte）<br/>可选尾帧图片。提供后将作为视频结束帧，实现首尾帧视频生成 |
| prompt | string | 是 | 文本长度限制：1 - 5000<br/>视频生成提示词，最大5000字符。启用推荐提示词(is_rec)时手动输入的提示词将被忽略 |
| resolution | string | 是 | ⟨bz_enum_json⟩["540P","720P","1080P"]⟨/bz_enum_json⟩<br/>输出分辨率，viduq3-pro默认720P |
| aspect_ratio | string | 是 | ⟨bz_enum_json⟩["16:9","9:16","4:3","3:4","1:1"]⟨/bz_enum_json⟩<br/>输出视频宽高比，不设置时由输入图片决定 |
| duration | number | 是 | 取值范围：1 ~ 16<br/>视频时长，单位秒，viduq3-pro可用范围1-16秒 |
