---
display_name: "Vidu Q3.Pro-首尾帧-官方版"
category: "FLF to Video"
manufacturer: "生数科技"
price: "540P: 438 金币/1秒 | 720P: 938 金币/1秒 | 1080P: 1000 金币/1秒"
price_url: "https://bizyair.cn/modelzoo/vidu-q3-pro-official/flf-to-video?tab=price"
benefit:
  rpd: 200
  rph: 60
  rpm: -1
description: |
  生数科技Q3.Pro官方版首尾帧生视频模型，高度还原人物样貌神态与形体特征，动态衔接流畅细腻无生硬感，画质细节丰富且风格高度统一，全程无人物畸变、漂移及画面撕裂问题，适用于高端虚拟人动态制作、精致二次元形象活化与专业人像创意视频创作场景。
tags: ["Vidu"]
---

# Vidu Q3.Pro-首尾帧-官方版

> **首尾帧生视频** | 厂商: 生数科技 | 模型: `vidu-q3-pro-official` | 类型: `flf-to-video`

生数科技Q3.Pro官方版首尾帧生视频模型，高度还原人物样貌神态与形体特征，动态衔接流畅细腻无生硬感，画质细节丰富且风格高度统一，全程无人物畸变、漂移及画面撕裂问题，适用于高端虚拟人动态制作、精致二次元形象活化与专业人像创意视频创作场景。

💰 **价格**: 540P: 438 金币/1秒 | 720P: 938 金币/1秒 | 1080P: 1000 金币/1秒  [查看详情](https://bizyair.cn/modelzoo/vidu-q3-pro-official/flf-to-video?tab=price)
...
async function submitTask() {
  const url = 'https://api.bizyair.cn/x/v1/modelzoo/tasks/openapi/vidu-q3-pro-official/flf-to-video';
  const payload = {
    "image": [
...
| 参数名 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| image | array | 是 | 支持格式：png、jpeg、jpg、webp<br/>单文件大小上限：50.0 MB（52428800 byte）<br/>最少上传数量：1<br/>最多上传数量：1<br/>输入首帧图片，作为视频生成的起始帧 |
| last_frame_image | array | 是 | 支持格式：png、jpeg、jpg、webp<br/>单文件大小上限：50.0 MB（52428800 byte）<br/>最少上传数量：1<br/>最多上传数量：1<br/>输入尾帧图片，作为视频生成的结束帧。首帧与尾帧宽高比须在0.8~1.25之间 |
...
| is_rec | boolean | 否 | 是否使用推荐提示词。启用后系统自动生成推荐提示词，每个任务额外消耗320金币 |
| audio | boolean | 是 | 是否启用音视频同步生成。true: 输出含对白和音效的视频；false: 输出无声视频。仅q3模型支持 |
| seed | number | 是 | 随机种子，-1表示自动生成 |
...
```json
{
  ...
  "outputs": {
    "videos": [
      "https://storage.bizyair.cn/outputs/zCPomtJUG2ncqUng.mp4"
    ]
  }
}
```
