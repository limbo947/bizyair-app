...
async function submitTask() {
  const url = 'https://api.bizyair.cn/x/v1/modelzoo/tasks/openapi/seedream-4-5-official/text-to-image';
  const payload = {
    "size": "4K",
    "prompt": "A futuristic neon-lit metropolis skyline at night, towering skyscrapers with holographic advertisements. Flying vehicles cruising through the rainy air, reflections of neon pink and cyan on wet asphalt streets below. Cyberpunk aesthetic, hyper-detailed, synthwave style, trending on ArtStation."
  };
...
```json
{
  ...
  "outputs": {
    "images": [
      "https://storage.bizyair.cn/outputs/QFDHegeoGOU8NYLO.jpeg"
    ]
  }
}
```
