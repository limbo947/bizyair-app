...
async function submitTask() {
  const url = 'https://api.bizyair.cn/x/v1/modelzoo/tasks/openapi/bza-image-f-k-max-base/text-to-image';
  const payload = {
    "aspect_ratio": "21:9",
    "prompt": "A nostalgic, filmic medium shot of a person dozing off on a velvet armchair in a cluttered, cozy living room. Soft, diffuse afternoon sunlight streams through a large window, illuminating dust motes and creating warm light pools on the aged, patterned rug. The person has messy, disheveled hair, is wearing comfortable, oversized clothes, and is curled up with a worn paperback book resting on their chest. A ceramic coffee mug, half-empty, sits on a small, bistro table next to a pair of old glasses. The background is filled with bookshelves and personal mementos, softly blurred with indistinct details. Shot on Kodak Portra 400H, intimate atmosphere, warm, textured, deeply atmospheric."
  };
...
```json
{
  ...
  "outputs": {
    "images": [
      "https://storage.bizyair.cn/outputs_examples/fluxkontext_7ef061f57e254f9f86c4e46f49c6eafb_1780311223_v9nqfawi.jpg"
    ]
  }
}
```
